...
/**
  * This is `com.google.protobuf.CodedInputStream` class
...
object CodedInputStream {
  ...
  def newInstance[T](value: T)(implicit bytes: Bytes[T]): CodedInputStream = {
    if (value.isInstanceOf[Array[T]]) newInstance(value.asInstanceOf[Array[Byte]]) else {
      newInstance(new ZhukovBytesInputStream(value))
    }
  }
  ...
  private final class ZhukovBytesInputStream[T](value: T)(implicit bytes: Bytes[T]) extends InputStream {
    val counter = new AtomicLong()
    def read(): Int = bytes.get(value, counter.getAndIncrement())
  }
...
class CodedInputStream private(buffer: Array[Byte], input: InputStream) {
  ...
  @throws[InvalidProtocolBufferException]
  def readBytes[T](implicit bytes: Bytes[T]): T = {
    val size: Int = readRawVarint32()
    if (size <= (bufferSize - bufferPos) && size > 0) {
      val result = bytes.copyFromArray(buffer, bufferPos, size)
      bufferPos += size
      result
    }
    else if (size == 0) {
      ...
      bytes.wrapArray(readRawBytesSlowPath(size))
    }
  }
  ...
  def readByteArray(): Array[Byte] = {
    val size: Int = readRawVarint32()
    if (size <= (bufferSize - bufferPos) && size > 0) {
      val result = buffer.slice(bufferPos, bufferPos + size)
      bufferPos += size
      result
    }
    else if (size == 0) {
