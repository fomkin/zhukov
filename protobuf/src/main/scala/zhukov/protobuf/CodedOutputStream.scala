...
/**
  * This is `com.google.protobuf.CodedOutputStream` class
  * rewritten in scala by ScalaPB project
  * https://github.com/scalapb/protobuf-scala-runtime.
  * In this project we remove dependency to ByteString.
  */
object CodedOutputStream {
  ...
  def computeBytesSize[T: Bytes](fieldNumber: Int, value: T): Int = {
  ...
  def computeBytesSizeNoTag[T](value: T)(implicit bytes: Bytes[T]): Int = {
    val size = bytes.size(value).toInt
...
class CodedOutputStream (output: OutputStream, buffer: Array[Byte]) {
  ...
  def writeBytes[T: Bytes](fieldNumber: Int, value: T): Unit = {
    writeTag(fieldNumber, WireFormat.WIRETYPE_LENGTH_DELIMITED)
  ...
  def writeBytesNoTag[T](value: T)(implicit bytes: Bytes[T]): Unit = {
    writeRawVarint32(bytes.size(value).toInt)
    writeRawBytes(value)
  }
  ...
  def writeRawBytes[T](value: T)(implicit bytes: Bytes[T]): Unit = {
    var offset = 0
    var length = bytes.size(value).toInt
    if (limit - position >= length) {
      // We have room in the current buffer.
      bytes.copyToArray(value, buffer, offset, position, length)
      position += length
      ...
      bytes.copyToArray(value, buffer, offset, position, bytesWritten)
      offset += bytesWritten
      length -= bytesWritten
      position = limit
      ...
      // and buffer offset == 0
      if (length <= limit) {
        bytes.copyToArray(value, buffer, offset, 0, length)
        position = length
      } else {
        val sl = bytes.slice(value, offset.toLong, (offset + length).toLong)
        // TODO can be optimized
        val ba = bytes.toArray(sl)
        output.write(ba)
      }
      totalBytesWritten += length
    }
